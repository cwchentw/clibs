...
ifeq ($(TRAVIS_OS_NAME),windows)
	powershell -NoProfile -Command "$(VSDEVCMD) -and $(CC) /Fe: $(TEST_BOOLEAN_EXEC) $(TEST_BOOLEAN_OBJ) $(CFLAGS)"
else
...
ifneq (,$(findstring $(CC),cl.exe))
ifeq ($(TRAVIS_OS_NAME),windows)
	powershell -NoProfile -Command "$(VSDEVCMD) -and $(CC) /Fe: $(TEST_MATH_EXEC) $(TEST_MATH_OBJ) $(CFLAGS)"
else
...
ifneq (,$(findstring $(CC),cl.exe))
ifeq ($(TRAVIS_OS_NAME),windows)
	powershell -NoProfile -Command "$(VSDEVCMD) -and $(CC) /Fe: $(TEST_STRING_EXEC) $(TEST_STRING_OBJ) $(CFLAGS)"
else
...
%.obj: %.c
ifeq ($(TRAVIS_OS_NAME),windows)
	powershell -NoProfile -Command "$(VSDEVCMD) -and $(CC) /c $< $(CFLAGS)"
else
	$(CC) /c $< $(CFLAGS)
endif
